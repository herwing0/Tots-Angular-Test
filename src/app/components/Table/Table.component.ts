  ...
  export class TableComponent implements OnInit {
    ...
    items: NewClient[] = [];
    $clientData!: Observable<NewClient>;
    ...
    addClient(action: any) {
      ...
      this.clientService.postClient(obj).subscribe({
        next: (newClient) => {
          this.items = [
            ...this.items,
            { id: newClient.response.id, firstname: newClient.response.firstname, lastname: newClient.response.lastname, email: newClient.response.email } as NewClient
          ];

          let data = new TotsListResponse();
          data.data = this.items;

          //sort para lograr que el último id venga siempre primero
          data.data = this.items.sort((a, b) => !!a.id && !!b.id ? (a.id < b.id) ? 1 : ((b.id < a.id) ? -1 : 0) : 1);
    ...
    openFormModal(editItem?: any) {
      ...
      // Con esta línea podemos lograr que edite y cree nuevos clientes al mismo tiempo sin demasiado esfuerzo y sin utilizar el /client/fetch
